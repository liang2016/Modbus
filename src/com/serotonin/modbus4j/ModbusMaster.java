...
import com.serotonin.modbus4j.msg.ReadHoldingRegisters_88Request;
import com.serotonin.modbus4j.msg.ReadHoldingRegisters_88Response;
...
abstract public class ModbusMaster extends Modbus {
    ...
    public PrintWriter getIoLog() {
        return ioLog;
    }

    public void setIoLog(PrintWriter ioLog) {
        this.ioLog = ioLog;
    }
    ...
    private <K> void sendFunctionGroup(ReadFunctionGroup<K> functionGroup, BatchResults<K> results,
        ...
        long time=functionGroup.getSlaveAndTime().getTime();
        ...
        else if (functionGroup.getFunctionCode() == FunctionCode.READ_HOLDING_REGISTERS_88)
            request = new ReadHoldingRegisters_88Request(slaveId, startOffset, length,time);//这里的时间是假设的为0
        ...
        for (KeyedModbusLocator<K> locator : functionGroup.getLocators()) {
            if (errorsInResults && response.isException())
                ...
                try {
                	if(response.getFunctionCode()==FunctionCode.READ_HOLDING_REGISTERS_88){
                		long[] times= null;
                		times=response.getTimes();
                		results.addTimes(locator.getKey(), times);
                	}
