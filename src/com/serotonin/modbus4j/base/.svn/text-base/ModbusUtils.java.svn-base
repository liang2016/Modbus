...
public class ModbusUtils {
    ...
    //添加检测crc用于EncapMessageResponse
    public static void checkCRC2(ByteQueue queue) throws ModbusTransportException {
    	ByteQueue data= new ByteQueue();
    	data.push(queue.tailPop());
    	data.push(queue.tailPop());

    	
    	ByteQueue tmpQueue = new ByteQueue();
    	int slaveId=popByte(queue);//取出slaveId
    	pushByte(tmpQueue,slaveId);//将slaveId放入一个新的Queue
    	tmpQueue.push(queue);//放入取出slaveId周后的queue
    	queue.clear();//清除?
    	int givenCrc = ((data.pop() & 0xff)|(data.pop() & 0xff) << 8);
        int calcCrc = ModbusUtils.calculateCRC2(tmpQueue);
        if (calcCrc != givenCrc){
            throw new ModbusTransportException("CRC mismatch: given=" + givenCrc + ", calc=" + calcCrc,
            		slaveId);
        }
    ...
    //添加检测crc用于EncapMessageResponse
    public static int calculateCRC2(ByteQueue queue) {
