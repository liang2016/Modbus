...
abstract public class ReadResponse extends ModbusResponse {
	private byte[] data;
	private long[] times;//定义时间,仅在88号功能中使用
	...
	protected void readResponse(ByteQueue queue) {
		...
		int numberOfBytes = enhanceGetNumberOfBytes(queue);
		enhanceReadResponse(queue);
	...
	abstract int enhanceGetNumberOfBytes(ByteQueue queue); // 取出numberOfBytes

	abstract void enhanceReadResponse(ByteQueue queue); // 取出时间
	abstract void enhanceWriteResponse(ByteQueue queue); // 取出时间

	@Override
	protected void writeResponse(ByteQueue queue) {
		ModbusUtils.pushByte(queue, data.length);
		enhanceWriteResponse(queue);
		queue.push(data);
	}
